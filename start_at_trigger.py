...
class World(object):
    ...
    def restart(self, args):
        ...
        # =========================================================================
        # ==== MODIFICATION START: Replaced the vehicle finding logic below =====
        # =========================================================================

        # 1. Get the blueprint library
        blueprint_library = self.world.get_blueprint_library()

        # --- Here you can change the vehicle model ---
        # Examples: 'vehicle.audi.tt', 'vehicle.ford.mustang', 'vehicle.bmw.grandtourer'
        vehicle_bp = blueprint_library.find('vehicle.tesla.model3')
        print(f"Selected vehicle: {vehicle_bp.id}")

        # --- Here you can change the initial location and rotation ---
        # x, y, z are coordinates; yaw is the heading direction (0=East, 90=South, 180=West, 270=North)
        spawn_location = carla.Location(x=250.0, y=-150.0, z=1.0)
        spawn_rotation = carla.Rotation(yaw=180.0)
        transform = carla.Transform(spawn_location, spawn_rotation)

        # 3. Attempt to spawn the vehicle at the specified transform
        print(f"Attempting to spawn vehicle at: {spawn_location}")
        self.player = self.world.try_spawn_actor(vehicle_bp, transform)

        # 4. If spawning failed (e.g., location is occupied), exit with an error
        if self.player is None:
            print("!!! ERROR: Could not spawn vehicle at the specified transform.")
            print("!!! Please check if the location is valid and not occupied.")
            sys.exit()
    ...
    def tick(self, clock, wait_for_repetitions):
        if len(self.world.get_actors().filter(self.player_name)) < 1:
            if not wait_for_repetitions:
                ...
                self.player = None
                self.destroy()
                self.restart(self.args) # Pass args on restart
